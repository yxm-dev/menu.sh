...
by [user360154](https://askubuntu.com/users/360154/user360154). The later pointed to [his webpage](https://web.archive.org/web/20180130222805/http://pro-toolz.net/data/programming/bash/Bash_fancy_menu.html)
...
2. create an array named `menu_options` or `menu_opt` with the options to be selected;
3. create an associative array `menu_commands` or `menu_cmd` with the commands to be passed to the options;
4. call the function `menu_sh` with `selected_menu_option` or `sel_menu_opt` as a variable.

* you don't need to declare the arrays. They were already declared in `menu.sh`.
